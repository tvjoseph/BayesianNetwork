...
for i = 1:size(E, 1),
    ...
    for j = 1:length(F),
        ...
        if (~isempty(indx)),
            ...
            % JMJPFU 
            % 24-Jan-2017
            ind = IndexToAssignment(1:length(F(j).val), F(j).card); % Gets the assignment matrix
            
            for k = 1:length(ind(:,indx)),
              % looping over the number of assignments
              if (ind(k,indx)!=x), % Checking if the assignment is not equal to evidence
                assg = ind(k,:); % get the vector of assignment
                F(j) = SetValueOfAssignment(F(j),assg,0); % Assign the value which has no evidence to zero
